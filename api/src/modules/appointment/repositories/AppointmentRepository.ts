...
export class AppointmentRepository implements IAppointmentRepository {
  ...
  async getAll(): Promise<AppointmentType[]>{
    try {
      const appointments = this.appointmentModel.find({deletedAt: null})
      return appointments
    } catch (error: any) {
  ...
  async getById(id: string): Promise<AppointmentType | null>{
    try {
      ...
      const appointment = await this.appointmentModel.findOne({_id: id, deletedAt: null});
      return appointment
    } catch (error: any) {
      throw new Error(error)
    }
  ...
  async update(id: string, appointmentData: AppointmentDTO): Promise<AppointmentType>{
    try {
      ...
      const updatedAppointment = await this.appointmentModel.findByIdAndUpdate(id, appointmentData, {new: true})
  ...
  async softDelete(id: string): Promise<AppointmentType>{
    try {
      ...
      const deletedAppointment = await this.appointmentModel.findByIdAndUpdate(id, { deletedAt: new Date()}, {new: true})
